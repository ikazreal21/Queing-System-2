...
<div class="home">
    <div class="top-header"><h1>Manage Requests</h1></div>
...
<tr data-request-id="<?= $req['id'] ?>" data-documents="<?= htmlspecialchars($req['documents']) ?>">
    <td><?= $req['id'] ?></td>
    ...
    <td>
        <form method="post" action="update_request.php" class="approve-form" style="display:inline;">
            <input type="hidden" name="request_id" value="<?= $req['id'] ?>">
            <input type="hidden" name="action" value="approve">
            <button type="button" class="action-btn approve-btn" data-request="<?= $req['id'] ?>">✓</button>
        </form>
        ...
        <table class="approve-table">
            <thead>
                <tr>
                    ...
                    <th class="countdown">Countdown</th>
                    <th>Actions</th>
                </tr>
            </thead>
            <tbody>
    ...
    <div class="modal-content-unique">
        ...
        <form id="walkin-form-unique" enctype="multipart/form-data" method="POST" action="process_walkin.php">
            ...
            <div class="document-columns-unique">
                <div class="document-left-unique">
                    <?php foreach (array_slice($documents, 0, ceil(count($documents)/2)) as $doc): ?>
                        <?php $days = (int)$doc['processing_days']; $claim_date = date('F j, Y', strtotime("+$days days")); ?>
                        <label class="doc-checkbox-unique" title="Processing: <?= $days; ?> day(s) • Claim on <?= $claim_date; ?>">
            ...
            <div class="upload-section-unique">
                ...
                <input type="file" name="attachment[]" id="attachment-unique" accept=".jpg,.jpeg,.png,.pdf" multiple>
            </div>
